...
namespace SIT.Core.Coop.Player
{
    public class Player_ApplyDamageInfo_Patch : ModuleReplicationPatch
    {
            ...
            )
        {
            ...
            Dictionary<string, object> dictionary = new Dictionary<string, object>();
            dictionary.Add("t", DateTime.Now.Ticks);
            //foreach(var f in PatchConstants.GetAllFieldsForObject(damageInfo))
            //{
            //    dictionary.Add($"d.{f.Name}", JsonConvert.SerializeObject(f.GetValue(damageInfo), new JsonSerializerSettings() { ReferenceLoopHandling = ReferenceLoopHandling.Ignore }));
            //}
            dictionary.Add("d", JsonConvert.SerializeObject(damageInfo, new JsonSerializerSettings() { ReferenceLoopHandling = ReferenceLoopHandling.Ignore }));
        ...
        public override void Replicated(EFT.Player player, Dictionary<string, object> dict)
        {
            ...
            try
            {
                ...
                //foreach(var k in dict.Keys.Where(x => x.StartsWith("d.")))
                //{
                //    var fieldName = k.Remove(0, 2);
                //    //PatchConstants.SetFieldOrPropertyFromInstance(damageInfo, fieldName, (dict[k], new JsonSerializerSettings() { ReferenceLoopHandling = ReferenceLoopHandling.Ignore })); )
                //}
                Enum.TryParse<EBodyPart>(dict["bpt"].ToString(), out var bodyPartType);
                var absorbed = float.Parse(dict["ab"].ToString());
                CallLocally.Add(player.Profile.AccountId, true);
                player.ApplyDamageInfo(damageInfo, bodyPartType, absorbed, null);
            }
            catch (Exception e)
            {
                Logger.LogInfo(e);
