...
4. you can find additional config options in `SIT_DIR\game\BepInEx\config\SIT.Core.cfg`, where you can, for instance, disable the spawn/kill feed in the bottom right corner of the screen while in raid. It is strongly recommended that all players use the same config options.